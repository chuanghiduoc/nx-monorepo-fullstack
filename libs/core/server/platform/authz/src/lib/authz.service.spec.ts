...
describe('roles', () => {
  ...
  it('combines several roles', () => {
    expect(authz.can(user(['member', 'owner']), 'organization.delete')).toBe(
      true,
    );
  });
...
describe('organization scope', () => {
  ...
  it('allows a resource in the principal’s own organization', () => {
    expect(authz.can(user(['owner']), 'note.update', { orgId: ORG })).toBe(
      true,
    );
  });
});
...
describe('api keys', () => {
  ...
  it('is limited to its own permissions, never the issuer’s', () => {
    ...
    expect(authz.can(key({ note: ['read'] }), 'organization.delete')).toBe(
      false,
    );
  });
...
describe('canAny / canAll', () => {
  ...
  it('canAll needs every one', () => {
    ...
    expect(authz.canAll(user(['member']), ['note.read', 'note.create'])).toBe(
      true,
    );
  });
});
...
describe('the registry', () => {
  ...
  it('parses only declared actions', () => {
    expect(parseAction('note.read')).toEqual({
      resource: 'note',
      verb: 'read',
    });
